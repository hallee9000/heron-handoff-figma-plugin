...
export const hasSymbolProperties = textNode => {
    ...
    return !![fontName, fontSize, lineHeight, textDecoration, letterSpacing, fills].filter(
...
export const getSingleTextStyle = (textNode, index?) => {
    ...
    const letterSpacing =
        index === undefined ? textNode.letterSpacing : textNode.getRangeLetterSpacing(index, index + 1);
